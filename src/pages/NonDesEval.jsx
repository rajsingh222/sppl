    ...
    <Box p={4}>
        ...
        >
            ...
            >
                ...
                {showUploader && (
                    <PdfUploader
                        api="https://spplindia.org/api/add_others_nde.php"
                        onUploadSuccess={() =>
                            toast({
...
const NDE = () => {
    ...
    const sppl_dpc_reports = parseUrlString(project.sppl_nde_reports).map((report) => ({
        name: getTitle(report),
        pdfUrl: report,
        icon: <LuFile size="1.5rem" color="#2B6CB0" />,
    }));

    const others_dpc_reports = parseUrlString(project.others_nde_reports).map((report) => ({
        name: getTitle(report),
        pdfUrl: report,
        icon: <LuFile size="1.5rem" color="#2B6CB0" />,
    }));
